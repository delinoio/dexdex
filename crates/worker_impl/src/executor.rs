...
/// Generates a random plan file name with a 6-character alphanumeric suffix.
///
/// The suffix is generated by the worker server before the planning prompt is
/// created, ensuring the filename is known ahead of time and can be looked up
/// directly after the agent finishes.
fn generate_plan_file_name() -> String {
...
impl<E: EventEmitter + 'static> LocalExecutor<E> {
    ...
    pub async fn execute_composite_task(&self, composite_task_id: Uuid) -> Result<(), String> {
        ...
        // Generate a random suffix for the PLAN.yaml filename before building the prompt.
        // This ensures the filename is deterministic from the worker server's perspective,
        // so we can look for the exact file after the agent finishes.
        let plan_file_name = generate_plan_file_name();
        info!(
            "Generated plan file name for composite task {}: {}",
            composite_task_id, plan_file_name
        );

        // Build the full planning prompt with PLAN.yaml format instructions
        let planning_prompt = build_planning_prompt(&composite_task.prompt, &plan_file_name);
        ...
        let handle = tokio::spawn(async move {
            ...
            match &result {
                ExecutionResult::Success { .. } => {
                    ...
                    // Read PLAN.yaml from the worktree before cleanup using the
                    // pre-determined filename
                    let plan_yaml_content = if let Some(ref worktree) = worktree_path {
                        Self::read_plan_yaml_from_worktree(worktree, &plan_file_name).await
                    } else {
                        warn!(
    ...
    ///
    /// The plan file name is pre-determined by the worker server before the
    /// planning prompt is created. This method reads the exact file, falling
    /// back to a glob search for `PLAN-*.yaml` if the expected file is not
    /// found (e.g., the agent chose a different name).
    async fn read_plan_yaml_from_worktree(
        worktree_path: &Path,
        plan_file_name: &str,
    ) -> Option<String> {
        // First, try the exact pre-determined filename
        let exact_path = worktree_path.join(plan_file_name);
        debug!("Looking for plan file at exact path: {:?}", exact_path);
        ...
        // Fallback: glob for PLAN-*.yaml files in case the agent used a different name
...
mod tests {
    ...
    #[test]
    fn test_generate_plan_file_name_format() {
        let name = generate_plan_file_name();
        assert!(name.starts_with("PLAN-"));
        assert!(name.ends_with(".yaml"));
        // "PLAN-" (5) + 6 chars + ".yaml" (5) = 16
        assert_eq!(name.len(), 16);
    }

    #[test]
    fn test_generate_plan_file_name_suffix_is_alphanumeric() {
        let name = generate_plan_file_name();
        let suffix = &name[5..11]; // Extract the 6-char suffix
        assert!(suffix.chars().all(|c| c.is_ascii_lowercase() || c.is_ascii_digit()));
    }

    #[test]
    fn test_generate_plan_file_name_uniqueness() {
        let name1 = generate_plan_file_name();
        let name2 = generate_plan_file_name();
