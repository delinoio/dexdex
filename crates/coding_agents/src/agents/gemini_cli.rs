...
impl GeminiCliAgent {
    ...
    /// Parses Gemini CLI's text output format.
...
impl Agent for GeminiCliAgent {
    ...
    async fn run(
        ...
        debug!("Running Gemini CLI with args: {:?}", args);
...
mod tests {
    ...
    fn test_parse_file_creation() {
        let agent = GeminiCliAgent::new();
        let events = agent.parse_output("Creating file `src/main.rs`");

        let has_file_change = events.iter().any(|e| {
            matches!(e, NormalizedEvent::FileChange { path, change_type: FileChangeType::Create, .. } if path == "src/main.rs")
        });
    ...
    fn test_parse_command_execution() {
        let agent = GeminiCliAgent::new();
        let events = agent.parse_output("Running `cargo build`");

        let has_command = events.iter().any(|e| {
            matches!(e, NormalizedEvent::CommandExecution { command, .. } if command.contains("cargo build"))
        });
        assert!(has_command);
    }
    ...
    fn test_parse_question() {
        let agent = GeminiCliAgent::new();
        let events = agent.parse_output("Do you want to proceed?");
