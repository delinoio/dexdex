...
When a user creates a CompositeTask, the executor generates a random suffix and instructs the planning agent to create a `PLAN-{suffix}.yaml` file that defines the task graph for execution. The suffix is determined by the worker server/executor before the planning prompt is created, ensuring the system knows the exact filename to read back.

## File Location

- **Filename**: `PLAN-{suffix}.yaml` where `{suffix}` is a 6-character hex string generated by the executor
...
1. User creates CompositeTask
        ▼
2. Executor generates random suffix and builds prompt with exact filename
        ▼
3. Planning agent creates PLAN-{suffix}.yaml
        ▼
4. Plan copied to repository root
        ▼
5. User reviews plan in UI
